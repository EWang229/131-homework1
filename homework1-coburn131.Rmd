...
Predictive Models: We use this if we want to "see which combo of features works best" and "predict Y with minimum reducible error" (lecture notes day 2). They are not focused on hypothesis tests. 
...
```{r}
mpg_numerical <- mpg %>% select(displ, cty, hwy)
corr_mpg <- cor(mpg_numerical)
corrplot(corr_mpg, method = "square", order = "alphabet", type = "lower")
```
The variables that are positively correlated are cty and hwy.  
The variables that are negatively correlated are displ and cty, and displ and hwy.  
These relationships make sense because if cty and hwy are strongly, positively correlated, then it makes sense that both of them would have a similar relationship for a given variable from the dataset.